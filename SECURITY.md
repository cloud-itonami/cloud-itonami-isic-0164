...
If you discover a security issue in cloud-itonami-isic-0164, **do not open a
...
This repository implements the **Governor** for seed-processing-for-propagation
...
1. **Hard violations in Governor rules must never be overridable** — a rule
   marked as HARD (e.g., `:already-processed`, `:germination-rate-below-minimum`,
   `:seed-borne-disease-detected`) blocks the proposal unconditionally
2. **Escalation gates must trigger without race conditions** — quality
...
- Include documentation of the seed-viability/quality rationale

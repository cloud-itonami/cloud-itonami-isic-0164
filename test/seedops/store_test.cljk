...
(deftest production-batch-test
  (testing "retrieve an existing batch"
    (let [batch-data {:seed-lot-type :maize/hybrid :moisture-percent 13.0}
...
(deftest log-batch-test
  (testing "logging a batch marks it as processed"
    (let [st {:batches {}}
          batch-data {:seed-lot-type :maize/hybrid}
  ...
  (testing "logging preserves batch data"
    (let [st {:batches {}}
          batch-data {:seed-lot-type :maize/hybrid :moisture-percent 13.0}
          result (store/log-batch st "batch-001" batch-data)]
      (is (= (:seed-lot-type (get-in result [:batches "batch-001"])) :maize/hybrid))
      (is (= (:moisture-percent (get-in result [:batches "batch-001"])) 13.0)))))

;; ──────────────────────── Shipment Finalization ──────────────────────

(deftest finalize-shipment-test
  (testing "finalizing a batch marks it as finalized"
    (let [st {:batches {"batch-001" {:seed-lot-type :maize/hybrid}}}
...
(deftest append-fact-test
  (testing "appending a fact increases ledger length"
    (let [st {:facts []}
          fact {:t :governor-hold :op :log-processing-batch}
